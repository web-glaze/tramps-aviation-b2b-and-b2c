...
import { MapPin, ChevronLeft, ChevronRight, Plane, Navigation } from "lucide-react";
import { cn } from "@/lib/utils";
import { popularApi, unwrap } from "@/lib/api/services";

const FALLBACK: any[] = [
  { name: "Goa", country: "India", tagline: "Beach Paradise", airportCode: "GOI", flightPrice: "From ₹2,499" },
  { name: "Mumbai", country: "India", tagline: "City of Dreams", airportCode: "BOM", flightPrice: "From ₹2,199" },
  { name: "Delhi", country: "India", tagline: "Heart of India", airportCode: "DEL", flightPrice: "From ₹1,899" },
  { name: "Bangalore", country: "India", tagline: "Silicon Valley", airportCode: "BLR", flightPrice: "From ₹1,299" },
  { name: "Jaipur", country: "India", tagline: "Pink City", airportCode: "JAI", flightPrice: "From ₹1,999" },
  { name: "Chennai", country: "India", tagline: "Gateway of South", airportCode: "MAA", flightPrice: "From ₹2,799" },
];

function CityCard({ city, isNearest, onClick }: { city: any; isNearest?: boolean; onClick: () => void }) {
  return (
    ...
    >
      <div className="h-28 sm:h-32 w-full relative overflow-hidden bg-slate-100 dark:bg-slate-800">
        {city.imageUrl
        ...
        {city.airportCode && (
          <div className="absolute top-2 right-2 bg-white/90 dark:bg-black/60 text-slate-700 dark:text-white text-[9px] font-bold px-1.5 py-0.5 rounded shadow-sm">
            {city.airportCode}
          </div>
        )}
        {isNearest && (
          <div className="absolute top-2 left-2 bg-primary text-white text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center gap-0.5">
            <Navigation className="h-2 w-2" /> Near You
          </div>
        )}
      </div>
      <div className="p-3.5">
        <p className="font-bold text-sm text-foreground leading-tight">{city.name}</p>
        <p className="text-[10px] text-muted-foreground mt-0.5 truncate">{city.tagline}</p>
        {city.flightPrice && (
          <div className="flex items-center gap-1 mt-2">
            <Plane className="h-2.5 w-2.5 text-primary flex-shrink-0" />
            <span className="text-[10px] font-semibold" style={{ color: "hsl(var(--brand-orange))" }}>{city.flightPrice}</span>
          </div>
        )}
        <div className="mt-2.5 pt-2 border-t border-border/40">
          <span className="text-[10px] text-primary font-semibold">Search flights →</span>
        </div>
      </div>
    </button>
  );
...
export function PopularCities({ preloaded }: { preloaded?: any[] }) {
  ...
  useEffect(() => {
    if (preloaded !== undefined) {
      ...
      popularApi.getCities()
        .then((r) => { const d = unwrap(r); setCities(Array.isArray(d) && d.length ? d : FALLBACK); })
    ...
    if (typeof navigator !== "undefined" && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (pos) => {
        try {
          ...
          const MAP: Record<string, string> = { Delhi: "DEL", "New Delhi": "DEL", Mumbai: "BOM", Bangalore: "BLR", Bengaluru: "BLR", Chennai: "MAA", Hyderabad: "HYD", Kolkata: "CCU", Pune: "PNQ", Ahmedabad: "AMD", Goa: "GOI", Jaipur: "JAI", Kochi: "COK", Lucknow: "LKO", Chandigarh: "IXC" };
          const code = MAP[city];
  ...
  const sorted = nearestCode ? [...cities].sort((a, b) => a.airportCode === nearestCode ? -1 : b.airportCode === nearestCode ? 1 : 0) : cities;
  ...
  const onCityClick = (c: any) => {
    ...
    if (c.airportCode) router.push(`/flights?from=${from}&to=${c.airportCode}&date=${d}&adults=1`);
    ...
    <section className="py-2">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 ml-10">
              ...
              {nearestCode && <span className="ml-1 text-primary font-medium">· Nearest first</span>}
            </p>
          </div>
        ...
        <div ref={ref} onScroll={update}
          ...
          {sorted.map((c, i) => (
            <CityCard key={c._id || c.name + i} city={c}
