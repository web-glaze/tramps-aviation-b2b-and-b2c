...
function CountryCard({ country, onClick }: { country:any; onClick:()=>void }) {
  ...
  return (
    <button onClick={onClick}
      className={cn("group flex-shrink-0 w-36 sm:w-44 text-left rounded-xl sm:rounded-2xl overflow-hidden", glassCard)}>
        ...
        {country.imageUrl
          ...
          : (
            <div className="w-full h-full flex items-center justify-center">
              <span className="text-5xl group-hover:scale-110 transition-transform duration-300 leading-none">{country.flag||"🌍"}</span>
            </div>
          )
        }
        {country.imageUrl && country.flag && (
          <div className="absolute top-1.5 right-1.5 text-xl drop-shadow">{country.flag}</div>
        )}
        {country.visaInfo && (
          <div className={`absolute bottom-1.5 left-1.5 text-[7px] sm:text-[8px] font-bold px-1.5 py-0.5 rounded ${visaBg}`}>
            {country.visaInfo}
          </div>
        )}
      </div>
      <div className="p-2.5 sm:p-3.5">
        <p className="font-bold text-xs sm:text-sm text-foreground leading-tight">{country.name}</p>
        <p className="text-[9px] sm:text-[10px] text-muted-foreground mt-0.5 truncate">{country.tagline}</p>
        {country.flightPrice && (
          <div className="flex items-center gap-1 mt-2">
            <Plane className="h-2 w-2 sm:h-2.5 sm:w-2.5 text-primary flex-shrink-0"/>
            <span className="text-[9px] sm:text-[10px] font-semibold" style={{color:"hsl(var(--brand-orange))"}}>
              {country.flightPrice}
            </span>
          </div>
        )}
        {country.currency && (
          <p className="text-[8px] sm:text-[9px] text-muted-foreground mt-0.5">💰 {country.currency}</p>
        )}
        <div className="mt-2.5 pt-2 border-t border-border/40">
          <span className="text-[9px] text-primary font-semibold">Explore flights →</span>
        </div>
      </div>
    </button>
  );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between mb-5 sm:mb-6">
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-1 ml-10">Visa info & flight prices included</p>
          </div>
        ...
        <div ref={ref} onScroll={update}
          ...
          {countries.map((c,i) => (
            <CountryCard key={c._id||c.name+i} country={c} onClick={()=>router.push("/flights")}/>
          ))}
        </div>
      </div>
    </section>
