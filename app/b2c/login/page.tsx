...
import { Plane, Eye, EyeOff, Phone, Mail, ArrowLeft, Loader2 } from "lucide-react";
...
function LoginContent() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const token = data?.access_token || data?.accessToken || data?.token || data?.data?.token;
      if (token && userData) {
      ...
      const mockUser = { id: `c_${Date.now()}`, name: email.split("@")[0] || "Customer", email, role: "customer" as const };
  ...
  const handleSendOtp = async () => {
    if (!phone || phone.length < 10) return toast.error("Enter valid 10-digit number");
    setOtpLoading(true);
    try {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mockUser = { id: `c_${Date.now()}`, name: `+91${phone}`, email: `${phone}@demo.com`, role: "customer" as const };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card border border-border rounded-2xl p-7 shadow-2xl">
          ...
          <div className="flex items-center gap-2.5 mb-7">
            ...
            <div>
              <p className="font-extrabold text-white text-base">TravelPro</p>
              <p className="text-xs text-muted-foreground">Customer Portal</p>
            </div>
          </div>

          <h2 className="text-2xl font-bold text-white mb-1">Sign In</h2>
          <p className="text-muted-foreground text-sm mb-6">Access your bookings and manage trips</p>

          {/* Tabs */}
          <div className="flex bg-muted/60 border border-border rounded-xl p-1 mb-6 gap-1">
            <button onClick={() => setTab("email")}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium rounded-lg transition-all ${
                tab === "email" ? "bg-blue-600 text-white shadow" : "text-muted-foreground hover:text-white"
              }`}>
              <Mail className="h-3.5 w-3.5" /> Email
            </button>
            <button onClick={() => setTab("otp")}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium rounded-lg transition-all ${
                tab === "otp" ? "bg-blue-600 text-white shadow" : "text-muted-foreground hover:text-white"
              }`}>
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <span className="bg-muted border border-border text-muted-foreground rounded-xl px-3 py-3 text-sm flex items-center">+91</span>
                  ...
                  <button type="button" onClick={handleSendOtp} disabled={otpLoading || countdown > 0}
                    ...
                    {otpLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : countdown > 0 ? `${countdown}s` : "Send OTP"}
                  </button>
                </div>
              </div>
              {otpSent && (
                <div>
                  <label className="text-xs text-muted-foreground font-medium block mb-1.5">6-Digit OTP</label>
          ...
          <div className="mt-5 text-center">
            <p className="text-sm text-muted-foreground">
              ...
              <Link href="/b2c/register" className="text-blue-400 hover:text-blue-300 font-medium transition-colors">
                Register free
              </Link>
            </p>
          </div>
        ...
        <p className="text-center text-xs text-slate-700 mt-4">
          Agent or Admin?{" "}
          <Link href="/b2b/login" className="text-muted-foreground hover:text-slate-300 transition-colors">Login here →</Link>
        </p>
      </div>
    </div>
  );
