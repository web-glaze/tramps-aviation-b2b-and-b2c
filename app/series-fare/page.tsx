    ...
    <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all hover:border-primary/30">
      ...
      <div className="p-4">
        <div className="flex items-center gap-3">
          ...
          >
            {(flight.airlineCode || flight.airline || "TA")
              .slice(0, 2)
              .toUpperCase()}
          </div>
          ...
          <div className="text-right pl-3 border-l border-border flex-shrink-0">
            ...
            <p className="text-2xl font-bold text-[#e44b0f] font-display">
              ₹
              {Number(
                flight.price || flight.fare?.totalFare || 0,
          ...
          <div className="mt-3 pt-3 border-t border-border/60 bg-muted/30 rounded-xl p-3 space-y-2 text-xs">
              ...
              <div>
                ...
                <div className="space-y-0.5 text-muted-foreground">
                  <p>
                    Base Fare: ₹
                    {Number(
                      flight.fare?.baseFare || flight.price || 0,
                    ).toLocaleString("en-IN")}
                  </p>
                  <p>
                    Taxes: ₹
                    {Number(flight.fare?.taxes || 0).toLocaleString("en-IN")}
                  </p>
                  <p className="font-semibold text-foreground">
                    Total: ₹
                    {Number(
                      flight.fare?.totalFare || flight.price || 0,
                    ).toLocaleString("en-IN")}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg px-3 py-2">
              <p className="text-amber-700 dark:text-amber-400 font-medium">
                📞 Contact Tramps Aviation for group/bulk bookings on this
                series fare.
              </p>
            </div>
          </div>
...
function BookModal({
  ...
  const url = `/series-fare?from=${from}&to=${to}&date=${date}`;
  return (
      ...
      <div className="relative bg-card border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <div className="text-center mb-5">
          ...
          <p className="text-muted-foreground text-sm">
            {from} → {to} · ₹{Number(flight.price || 0).toLocaleString("en-IN")}{" "}
            per person
          </p>
        </div>
        <div className="space-y-3">
          <button
            onClick={() =>
              router.push(`/b2c/login?redirect=${encodeURIComponent(url)}`)
            }
        ...
        <p className="text-xs text-center text-muted-foreground mt-4">
          ...
          <a href="tel:+911800001234" className="text-primary hover:underline">
            call us
          </a>
        </p>
      </div>
...
function SeriesFarePage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setFlights(data?.data?.data || data?.data || data?.results || []);
      setSearched(true);
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Exclusive Tramps Aviation bulk & group flight inventory
              </p>
            </div>
          </div>
          ...
          <div className="mt-3 flex items-start gap-2 text-xs text-muted-foreground bg-muted/40 rounded-xl px-4 py-2.5 border border-border/60">
            ...
            <p>
              <strong className="text-foreground">Series Fares</strong> are
              Tramps Aviation's exclusive bulk inventory — separate from regular
              TBO fares. These offer special rates for group and series
              bookings. Contact us for volume discounts.
            </p>
          </div>
        </div>
      </div>
        ...
        {!loading && searched && flights.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground text-sm max-w-md mx-auto mb-6">
              ...
              on <strong>{date}</strong>. Try a different date or route, or
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <button
                onClick={() =>
                  router.push(
                    `/flights?from=${from}&to=${to}&date=${date}&adults=${adults}`,
                  )
                }
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="font-bold font-display">
                  {flights.length} Series Fare{flights.length > 1 ? "s" : ""}{" "}
                  Found
                </h2>
            ...
            <div className="mt-6 bg-primary/5 border border-primary/20 rounded-2xl p-5 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-3">
                Contact Tramps Aviation directly for group rates and series
                booking discounts.
              </p>
              <a
                href="tel:+911800001234"
                className="inline-flex items-center gap-2 px-5 py-2 bg-primary text-primary-foreground rounded-xl font-semibold text-sm hover:opacity-90 transition-all"
              >
