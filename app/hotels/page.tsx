      ...
      <div className="p-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs text-muted-foreground">
                · {(hotel.reviews || 0).toLocaleString("en-IN")} reviews
              </span>
            </div>
            {/* Amenities */}
            <div className="flex flex-wrap gap-1.5">
              {(hotel.amenities || []).slice(0, 4).map((a: string) => {
                ...
                return (
                  <span
                    key={a}
                    className="flex items-center gap-1 text-[10px] text-muted-foreground bg-muted px-2 py-1 rounded-lg border border-border"
                  >
          ...
          <div className="flex sm:flex-col items-center sm:items-end justify-between sm:flex-shrink-0 pt-3 sm:pt-0 border-t sm:border-0 border-border gap-3">
            <div className="sm:text-right">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                {hotel.mealPlan === "BREAKFAST_INCLUDED"
                  ? "Breakfast included"
                  : "Room only"}
              </p>
              ...
              <AgentEarnBadge
                totalAmount={total}
                commissionPercent={Number(hotel?.commissionPercent ?? 5)}
                commissionAmount={hotel?.commissionAmount}
                className="mt-1.5"
              />
            </div>
            <button
        ...
        {exp && (
          <div className="mt-3 pt-3 border-t border-border space-y-4">
            ...
            {/* Agent fare & commission breakdown (renders nothing for customers) */}
            <div className="bg-muted/40 rounded-xl p-4 border border-border/40">
              ...
              <AgentCommissionBreakdown
                ...
                productType="hotel"
              />
              {/* Customer fallback (simple breakdown) */}
              {role !== "agent" && (
                <div className="space-y-1.5 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">
                      ₹{ppn.toLocaleString("en-IN")} × {nights} night
                      {nights > 1 ? "s" : ""}
                    </span>
...
function HotelsContent() {
  ...
  const filtered = hotels.filter(
    (h) => filterStar === "all" || h.stars >= parseInt(filterStar),
  );
              ...
              <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="search-input-date w-full"
                  />
                </div>
                <div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6">
        {searched && !loading && hotels.length > 0 ? (
          <div className="flex gap-5">
            {/* Filter sidebar */}
            <div
              className="w-48 flex-shrink-0 hidden lg:block custom-scrollbar"
              style={{
                position: "sticky",
                top: "72px",
                  ...
                  <div className="space-y-1">
                    {[
                      ...
                      <button
                        ...
                        className={cn(
                          "w-full text-left text-xs px-3 py-2 rounded-lg transition-all font-medium flex items-center gap-2",
                          filterStar === v
            ...
            {loading && (
              <div className="space-y-3">
                {[1, 2, 3].map((n) => (
                  <div
                    key={n}
                    className="bg-card border border-border rounded-2xl p-5 animate-pulse shadow-sm h-32"
                  />
                ))}
            ...
            {!loading && !searched && (
              <div className="text-center py-28">
                ...
                <p className="text-muted-foreground text-sm max-w-xs mx-auto">
                  Enter a city and dates to search hotels
                </p>
              </div>
            )}
            {searched && !loading && hotels.length === 0 && (
