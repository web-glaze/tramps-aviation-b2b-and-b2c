...
export default function B2BLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!identifier.trim()) { toast.error("Please enter your Email, Agent ID, or Phone number"); return; }
    setLoading(true);
    try {
      ...
      setAuth({ id: agent.id || agent._id, name: agent.contactPerson || agent.agencyName, email: agent.email, role: "agent", kycStatus, status: agent.status, agencyName: agent.agencyName, agentId: agent.agentId, walletBalance: agent.walletBalance, kycRejectionReason: agent.kycRejectionReason } as any, token);
      if (kycStatus === "approved" || agent.status === "active") { toast.success(`Welcome back, ${agent.agencyName}!`); router.push("/b2b/dashboard"); return; }
      toast.info("Please complete your KYC to activate your account."); router.push("/b2b/kyc");
    } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="h-11 w-11 rounded-xl overflow-hidden bg-white border border-border flex-shrink-0">
            <Image src="/logo.jpg" alt="Tramps Aviation" width={44} height={44} className="h-11 w-11 object-contain" />
          </div>
          <div>
            <h1 className="font-bold text-lg text-foreground leading-none">Tramps Aviation B2B</h1>
            <p className="text-xs text-muted-foreground mt-0.5">Agent Portal</p>
          </div>
        </div>

        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
          <h2 className="text-2xl font-bold text-foreground mb-1">Agent Sign In</h2>
          <p className="text-sm text-muted-foreground mb-6">Use your Email, Agent ID (TRV-XXXXX), or Phone number</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <div className="flex gap-1.5 flex-wrap mt-2">
                {[{ label: "Email", example: "agent@agency.com" },{ label: "Agent ID", example: "TRV-00001" },{ label: "Phone", example: "9876543210" }].map(({ label, example }) => (
                  <span key={label} className="text-[10px] text-muted-foreground bg-muted border border-border rounded-md px-2 py-0.5">
                    {label}: <span className="text-foreground/60">{example}</span>
                  </span>
                ))}
              </div>
            </div>
          ...
          <div className="mt-5 pt-5 border-t border-border text-center space-y-2">
            ...
            <p className="text-xs">
              <Link href="/b2b/forgot-password" className="text-muted-foreground hover:text-foreground transition-colors">Forgot password?</Link>
            </p>
          </div>
        </div>
      </div>
