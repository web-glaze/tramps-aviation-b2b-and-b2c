...
import { Plane, Eye, EyeOff, ArrowLeft, CheckCircle, Clock, XCircle, Upload, Loader2, AlertTriangle } from "lucide-react";
...
interface KycScreen {
  visible: boolean; status: KycStatus; agencyName: string
  contactPerson: string; rejectionReason?: string
}
...
export default function B2BLoginPage() {
  ...
  const [kyc, setKyc] = useState<KycScreen>({ visible: false, status: "pending", agencyName: "", contactPerson: "" });
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setKyc({ visible: true, status: kycStatus, agencyName: agent.agencyName, contactPerson: agent.contactPerson, rejectionReason: agent.kycRejectionReason });
    } catch (err: any) {
  ...
  if (kyc.visible) {
    const cfg: Record<KycStatus, any> = {
      pending:      { icon: <AlertTriangle style={{ width: 32, height: 32, color: "#f59e0b" }} />, title: "KYC Not Submitted", color: "#f59e0b", borderColor: "rgba(245,158,11,0.3)", bg: "rgba(245,158,11,0.08)", msg: "Please upload your KYC documents to activate your account." },
      submitted:    { icon: <Clock style={{ width: 32, height: 32, color: "#60a5fa" }} />, title: "Documents Under Review", color: "#60a5fa", borderColor: "rgba(96,165,250,0.3)", bg: "rgba(96,165,250,0.08)", msg: "Your documents are being reviewed (24–48 hours)." },
      under_review: { icon: <Clock style={{ width: 32, height: 32, color: "#c084fc" }} />, title: "KYC Under Review", color: "#c084fc", borderColor: "rgba(192,132,252,0.3)", bg: "rgba(192,132,252,0.08)", msg: "Admin is reviewing your documents. You'll be notified by email." },
      approved:     { icon: <CheckCircle style={{ width: 32, height: 32, color: "#4ade80" }} />, title: "KYC Approved", color: "#4ade80", borderColor: "rgba(74,222,128,0.3)", bg: "rgba(74,222,128,0.08)", msg: "Your KYC is approved! Welcome aboard." },
      rejected:     { icon: <XCircle style={{ width: 32, height: 32, color: "#f87171" }} />, title: "KYC Rejected", color: "#f87171", borderColor: "rgba(248,113,113,0.3)", bg: "rgba(248,113,113,0.08)", msg: "Your KYC was rejected. Please re-upload corrected documents." },
    };
        ...
        <div style={{ width: "100%", maxWidth: "28rem" }}>
          <div style={{ ...s.card, border: `1px solid ${c.borderColor}` }}>
            <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
              <div style={{ width: 64, height: 64, borderRadius: "50%", background: c.bg, border: `1px solid ${c.borderColor}`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1rem" }}>{c.icon}</div>
              <h2 style={{ fontSize: "1.25rem", fontWeight: 700, color: c.color, margin: "0 0 0.25rem" }}>{c.title}</h2>
              <p style={{ fontSize: "0.875rem", color: "#64748b" }}>{kyc.agencyName}</p>
            </div>
            <p style={{ fontSize: "0.875rem", textAlign: "center", background: "#0f172a", borderRadius: "0.75rem", padding: "0.75rem", color: "#94a3b8", marginBottom: "1.25rem" }}>{c.msg}</p>
            {kyc.status === "rejected" && kyc.rejectionReason && (
              <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: "0.75rem", padding: "0.75rem", marginBottom: "1.25rem" }}>
                <p style={{ fontSize: "0.75rem", fontWeight: 600, color: "#f87171", marginBottom: "0.25rem" }}>Rejection Reason:</p>
                <p style={{ fontSize: "0.875rem", color: "#fca5a5" }}>{kyc.rejectionReason}</p>
              </div>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
              {(kyc.status === "pending" || kyc.status === "rejected") && (
                <Link href="/b2b/kyc" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", width: "100%", background: "#2563eb", color: "white", borderRadius: "0.75rem", padding: "0.75rem", fontSize: "0.875rem", fontWeight: 600, textDecoration: "none" }}>
                  <Upload style={{ width: 16, height: 16 }} />
                  {kyc.status === "rejected" ? "Re-upload KYC Documents" : "Upload KYC Documents"}
                </Link>
              )}
              {(kyc.status === "submitted" || kyc.status === "under_review") && (
                <div style={{ textAlign: "center", color: "#60a5fa", fontSize: "0.875rem", padding: "0.75rem" }}>⏳ Please wait for admin review</div>
              )}
              {kyc.status === "approved" && (
                <Link href="/b2b/dashboard" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", width: "100%", background: "#16a34a", color: "white", borderRadius: "0.75rem", padding: "0.75rem", fontSize: "0.875rem", fontWeight: 600, textDecoration: "none" }}>
                  <CheckCircle style={{ width: 16, height: 16 }} /> Go to Dashboard
                </Link>
              )}
              <button onClick={() => setKyc({ visible: false, status: "pending", agencyName: "", contactPerson: "" })}
                style={{ width: "100%", background: "#0f172a", border: "1px solid #1e293b", color: "#94a3b8", borderRadius: "0.75rem", padding: "0.75rem", fontSize: "0.875rem", fontWeight: 500, cursor: "pointer" }}>
                ← Back to Login
              </button>
            </div>
          </div>
