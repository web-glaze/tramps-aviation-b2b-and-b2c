...
import {
  Phone, Mail, MessageSquare, CheckCircle, FileText,
  Wallet, Plane, HelpCircle, ExternalLink, ChevronDown, ChevronUp,
} from "lucide-react";
...
export default function B2BHelpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) { setRefId(data.referenceId || ""); setSubmitted(true); }
    ...
    <div className="space-y-6">
        ...
        {[
          {
            ...
            sub: "Mon–Sat, 9AM–6PM", href: phone.startsWith("tel:") ? phone : `tel:${phone}`,
            color: "text-green-600 bg-green-50",
          },
          {
            icon: Mail, label: "Email Support", value: email,
            sub: "Response within 24 hours", href: `mailto:${email}`,
            color: "text-blue-600 bg-blue-50",
          },
          {
            icon: MessageSquare, label: "Submit Ticket", value: "Use the form below",
            sub: "Track with reference ID", href: "#support-form",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-6">
          ...
          <div className="space-y-2">
            {AGENT_FAQS.map((faq, i) => (
              <div key={i} className="border border-border rounded-xl overflow-hidden">
                <button className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-muted/50 transition-colors"
          ...
          <a href="/cms/faq" target="_blank" rel="noopener noreferrer"
        ...
        {/* Support Form */}
        <div id="support-form" className="bg-card border border-border rounded-2xl p-6">
          {submitted ? (
            <div className="text-center py-8">
              <div className="w-16 h-16 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-4">
                <CheckCircle className="h-8 w-8 text-green-600" />
              </div>
              <h3 className="font-bold text-foreground mb-2">Ticket Submitted!</h3>
              ...
              {refId && (
                <div className="p-3 bg-primary/10 rounded-xl mb-4">
                  ...
                  <p className="text-lg font-bold font-mono text-primary">#{refId}</p>
                </div>
              )}
              <button onClick={() => { setSubmitted(false); setForm(f => ({ ...f, subject: "", message: "", bookingRef: "" })); }}
            ...
            <>
              <h2 className="font-bold text-base text-foreground mb-4 flex items-center gap-2">
                <FileText className="h-4 w-4 text-primary" /> Submit Support Ticket
              </h2>
              <form onSubmit={handleSubmit} className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-foreground mb-1">Agent ID</label>
                    <input type="text" value={form.agentId} onChange={e => set("agentId", e.target.value.toUpperCase())}
                      placeholder="TAHP00001" readOnly={!!user?.agentId}
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground mb-1">Issue Type</label>
                ...
                {(form.type === "booking_help" || form.type === "refund") && (
                  <div>
                    <label className="block text-xs font-semibold text-foreground mb-1">Booking Reference</label>
                    <input type="text" value={form.bookingRef} onChange={e => set("bookingRef", e.target.value.toUpperCase())}
      ...
      {/* Useful links */}
      <div className="bg-card border border-border rounded-2xl p-6">
        <h2 className="font-bold text-base text-foreground mb-4">📚 Useful Resources</h2>
