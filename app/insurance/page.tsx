...
import { Shield, CheckCircle, Star, ArrowRight, RefreshCcw, Users, Building2 } from "lucide-react";
...
const PLANS = [
  {
    ...
    price:149, badge:"", highlight:false,
  ...
  },
  {
    ...
    price:399, badge:"Most Popular", highlight:true,
  ...
  },
  {
    ...
    price:799, badge:"Best Value", highlight:false,
...
export default function InsurancePage() {
  ...
  const handleBuy = (plan: any) => {
    if (isAuthenticated && role === "agent") {
      ...
      setTimeout(()=>{ setBuying(null); window.location.href = "/b2b/bookings"; }, 2000);
      return;
    }
    if (isAuthenticated) {
      setBuying(plan.id);
      toast.success(`${plan.name} selected — redirecting to payment…`);
      setTimeout(()=>{ setBuying(null); router.push("/b2c/my-trips"); }, 2000);
      return;
    }
    setRoleModal(true);
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-background">
      ...
      <div className="search-hero">
        <div className="max-w-5xl mx-auto px-4 py-10 text-center">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Travel Insurance</h1>
          <p className="text-white/80 text-sm max-w-lg mx-auto">
            Comprehensive protection for your journeys — from domestic weekend trips to worldwide adventures
          </p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {plans.map((plan)=>(
            <div key={plan.id}
              ...
              {/* Badge */}
              {plan.badge && (
                <div className={cn("px-4 py-1.5 text-center text-[11px] font-bold uppercase tracking-wider",
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center",
                    ...
                    <Shield className={cn("h-5 w-5", plan.highlight ? "text-primary" : "text-muted-foreground")}/>
                  </div>
                  <div>
                ...
                <div className="mb-5">
                  ...
                  <p className="text-4xl font-black leading-none" style={{color:"hsl(var(--brand-orange))"}}>
                    ₹{plan.price}
                  </p>
                ...
                <div className="space-y-2 mb-6">
                  {plan.coverage.map(item=>(
                    <div key={item} className="flex items-start gap-2">
                      <CheckCircle className={cn("h-3.5 w-3.5 flex-shrink-0 mt-0.5",
                        plan.highlight ? "text-primary" : "text-emerald-600 dark:text-emerald-400"
                      )}/>
                ...
                <button onClick={()=>handleBuy(plan)} disabled={buying===plan.id}
                  className={cn("w-full h-11 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all shadow-sm active:scale-[.98]",
                    plan.highlight
                      ? "btn-primary"
        ...
        <div className="mt-12 grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={t} className="bg-card border border-border rounded-xl p-4 text-center shadow-sm">
